const char* handleIndexPage = R"rawliteral(
<!DOCTYPE HTML><html>
<head><title>Nodemcu web</title></head>
<body><h1>Nodemcu web server with socket</h1></body></html>
)rawliteral";

String SeeClients(int num, int size) {
  ...
  html += "<h1 id='header'>" + String(size) + "</h1>";
  html += "<div id='main-container'>";
  
  // Loop through each client and generate buttons
  for (int i = 0; i < num; i++) {
    html += "<div id='" + i + "' class='client-button'>";
    html += "<p>Client #" + String(i + 1) + "</p>";
    html += "<div class='shutdown-button' onclick='shutdownClient(" + String(i + 1) + ")'>Shutdown</div>";
  ...
  html += "<script>";
  html += "function shutdownClient(id) { fetch('/shutdown?id=' + id).then(response => response.text()).then(data => { alert(data); })}";
